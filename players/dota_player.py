...
class DotaPlayer:
    ...
    def save_data(self, output_path, overwrite_data=True):
        ...
        if self.player_info:
            with open(os.path.join(player_dir, "player_info.json"), 'w', encoding='utf-8') as f:
                json.dump(self.player_info, f, ensure_ascii=False, indent=4)

        # Save player_matches if available
        if self.player_matches:
            with open(os.path.join(player_dir, "player_matches.json"), 'w', encoding='utf-8') as f:
                json.dump(self.player_matches, f, ensure_ascii=False, indent=4)
    ...
    def get_player_info(self):
        ...
        if self.player_info_response.status_code != 200:
    ...
    def get_matches(self):
        ...
        if self.player_matches_response.status_code != 200:
            logger.error("Error querying player info for ID %s", self.account_id)
            logger.error("Check if account ID %s is valid", self.account_id)
            return False
